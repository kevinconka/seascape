...
- **A camera's bearing is not the sum of the two yaws.** The rig composes `Rz(-pod yaw) Rx(rig pitch) Rz(-camera yaw)`, so the rig's pitch sits between the two yaws and shifts an off-axis camera's azimuth -- 0.108 deg at -5 deg of pitch, nine pixels at 4K. A centre camera is exact, which is why the sum looks right. `scene.boresight_deg` reads the achieved bearing off `matrix_world`; `Mount.nominal_bearing_deg` is only what the scenario asked for.
