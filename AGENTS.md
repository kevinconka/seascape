...
- **Poses are functions of time**, sampled into a key per frame by `scene._animate`. Ground truth records a timestamp, not a frame index: sensors run at different rates, so frame *n* is not one instant.
