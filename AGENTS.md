...
- **4D noise carries less slope than 3D.** Switching `noise_dimensions` with the same inputs flattens the waves by about 12%, with no error, so each has its own measured `NOISE_SLOPE_PER_UNIT`.
