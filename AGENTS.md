...
- **The sequencer tones its strips through the scene's view transform.** The factory AgX takes a png's white to about 195 and its black to about 8. `Standard` passes a picture through.
- **A strip frame that is missing encodes as black.** Blender reports nothing, so check the files exist first.
